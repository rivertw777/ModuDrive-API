...
| `common:infrastructure:outbox`      | Transactional outbox: producers call `OutboxEventRecorder.record(queue, key, event)` instead of `SqsTemplate.send`, which writes an `outbox_event` row in the caller's transaction; a scheduled relay sends it to SQS (key → FIFO `MessageGroupId`, row id → `MessageDeduplicationId`) and deletes it; outages stop the batch and retry, while a message SQS rejects outright (400) is parked (`failed_at`) (`SKIP LOCKED`; each service has its own `outbox_event` table in its own database). Used by member/file-service — publish from `BEFORE_COMMIT`, not `AFTER_COMMIT` |
