...
| `common:event`                      | Event DTOs + logical queue names (`*Queues`; the SQS adapter appends the broker's `.fifo` suffix) for async cross-service messaging: mail (`VerificationMailRequested`, `ShareInviteMailRequested`, `MailQueues` — member/file-service produce, mail-service consumes), notification (`FileSharedNotified`, `NotificationQueues` — file-service produces, notification-service consumes), member (`MemberSignedUp`, `MemberQueues` — member-service produces, file-service consumes) |
