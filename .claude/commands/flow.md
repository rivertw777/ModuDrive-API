...
5. Run `./gradlew build` to verify the change compiles and all tests pass. If it fails, fix the underlying issue and rerun until it passes.
6. Run `/code-review` and `/security-review` against the current diff. Fix any findings they surface; if a fix changes code, rerun `./gradlew build` before continuing.
