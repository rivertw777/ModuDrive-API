...
class MailEventListener {
    ...
    @SqsListener(MailDestinations.VERIFICATION_REQUESTED + SqsQueues.FIFO_SUFFIX)
    void onVerificationRequested(VerificationMailRequested event,
        ...
        if (processedEvents.isProcessed(MailDestinations.VERIFICATION_REQUESTED, deduplicationId)) {
            return;
        }
        sendVerificationMailUseCase.sendVerificationMail(
                new SendVerificationMailCommand(event.email(), event.verificationCode()));
        processedEvents.markProcessed(MailDestinations.VERIFICATION_REQUESTED, deduplicationId);
    }

    @SqsListener(MailDestinations.SHARE_INVITE_REQUESTED + SqsQueues.FIFO_SUFFIX)
    void onShareInviteRequested(ShareInviteMailRequested event,
                                @Header(MessageSystemAttributes.SQS_MESSAGE_DEDUPLICATION_ID_HEADER) String deduplicationId) {
        if (processedEvents.isProcessed(MailDestinations.SHARE_INVITE_REQUESTED, deduplicationId)) {
            return;
        }
        ...
        processedEvents.markProcessed(MailDestinations.SHARE_INVITE_REQUESTED, deduplicationId);
    }
}
