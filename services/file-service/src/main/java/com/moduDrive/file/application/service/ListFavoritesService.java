...
class ListFavoritesService implements ListFavoritesUseCase {
    ...
    public List<FileView> listFavorites(ListFavoritesCommand command) {
        ...
        // trashed, and for a non-owned file still actually shared (a star outlives a revoked
        // share and "즐겨찾기" must reflect what they can open now).
        return fileFavoritePort.favoriteFileIds(userId).stream()
                ...
                .filter(file -> file.getOwnerId().equals(userId)
                ...
                .map(file -> {
                    ...
                    return file.getOwnerId().equals(userId)
                            ? FileView.owned(file)
                            : FileView.shared(file, fileAccessGuard.effectiveRole(file, userId));
                })
                .toList();
    }
}
