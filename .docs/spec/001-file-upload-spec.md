...
> - WEB: 9장은 rivertw777/ModuDrive-WEB#207에서 개발할 예정입니다. 그 전까지 WEB은 [부록 A](#부록-a-web-207-이전-동작)대로 동작합니다. 파일마다 `POST /api/v1/files/metadata`를 호출하고, 폴더 업로드는 지원하지 않습니다.
>
...
충돌은 **최상위 항목에서만** 봅니다.
- 하위 항목은 새로 만드는 폴더 안에 들어가므로 충돌할 수 없습니다.
- 폴더끼리 합치는 일(병합)은 하지 않습니다 (구글 드라이브와 같음).
- 충돌 기준은 **같은 네임스페이스·같은 경로·같은 이름의 활성 항목**입니다 (DB 유니크 `uk_file_namespace_path_active_name`). 휴지통 항목은 충돌로 보지 않습니다.

| 최상위 항목 | 기존 항목 | 동작 |
|---|---|---|
| 폴더 | 폴더 | 묻지 않고 **번호 붙임** (`사진 (1)`) |
| 폴더 ↔ 파일 (종류가 다름) | | 묻지 않고 **번호 붙임** |
| 파일 | 파일 | `resolutions`에 선택이 있으면 그대로 처리. 없으면 **409** |

### 409 응답

선택이 없는 파일 충돌이 **하나라도** 있으면 아무것도 만들지 않고, 충돌한 이름을 **전부 모아** 한 번에 돌려줍니다.

```
409 FILE_BATCH_CONFLICT  "같은 이름의 파일이 이미 있습니다."
...
| 값 | 동작 |
|---|---|
| `REPLACE` | 소유자인지 확인한 뒤 기존 파일을 `PENDING`으로 되돌림(`restartUpload`) → 같은 `fileId`로 새 버전 업로드. 파일 ID·공유 설정·이전 버전 유지 |
| `KEEP_BOTH` | 번호를 붙여 새 파일로 만듦 |
| `SKIP` | 그 파일을 배치에서 뺌 |
...
2. 사전 검사
   - **5GB 초과 파일은 `items`에서 빼고**, 패널에 실패(`5GB 초과`)로 표시합니다. 나머지는 계속 진행합니다.
