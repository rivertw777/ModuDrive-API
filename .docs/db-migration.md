...
Hibernate `ddl-auto=update`로 스키마를 자동 관리하던 방식을 Flyway 버전 마이그레이션으로 바꿨다. 기대 효과는 다음과 같다.

- **모든 스키마 변경 가능** — 컬럼 삭제·이름 변경·타입 변경·NOT NULL 추가, 데이터 이전까지 SQL로 직접 다룬다.
- **변경 이력 관리** — 어떤 DB에 어떤 버전까지 적용됐는지 기록되고, 모든 환경이 같은 순서로 같은 스키마에 도달한다.
- **리뷰 가능** — 실행될 SQL이 PR에 그대로 드러난다.
- **운영 안정성** — 앱이 운영 DB를 임의로 바꾸지 않는다. 검증된 SQL만 적용되고, 앱은 스키마가 맞는지 확인만 한다.
- **테스트 신뢰도** — 테스트도 운영과 같은 마이그레이션으로 만든 스키마 위에서 돈다.

Flyway는 순수 SQL 파일과 버전 번호만으로 동작해 단순하고, Spring Boot가 기본 지원한다.
...
| `.docker/postgres/postgres_init.sh` | DB와 서비스별 계정 생성만 담당. 테이블·데이터는 만들지 않음 |
